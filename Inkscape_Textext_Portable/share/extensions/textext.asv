...
class LatexConverterBase(object):
    ...
    def __init__(self, document):
       ...
       # self.tmp_path = tempfile.mkdtemp()
        self.tmp_path = os.path.splitdrive(os.getcwd())[1] +'\\'
        self.tmp_base = 'tmp'
    ...
    def tex_to_pdf(self, latex_text, preamble_file):
        ...
        #exec_command(['C:/Newfolder/MiKTeX29/miktex/bin/pdflatex.exe', self.tmp('tex')] + latexOpts)
        strMiktex="%sMiKTeX29/miktex/bin/pdflatex.exe" % (self.tmp_path)
        exec_command([strMiktex, self.tmp('tex')] + latexOpts)
...
class SkConvert(PdfConverterBase):
    ...
    def pdf_to_svg(self):
        ...
        strPstoedit="%spstoedit/pstoedit.exe" % (self.tmp_path)
        exec_command([strPstoedit, '-f', 'sk',
    ...
    def available(cls):
        ...
        #strPstoedit="%spstoedit/pstoedit.exe" % (self.tmp_path)
        #out = exec_command([strPstoedit], ok_return_value=None)
...
class PstoeditPlotSvg(PdfConverterBase):
    ...
    def pdf_to_svg(self):
        ...
        strPstoedit="%spstoedit/pstoedit.exe" % (self.tmp_path)
        exec_command([strPstoedit, '-f', 'plot-svg',
    ...
    def available(cls):
        ...
        #strPstoedit="%spstoedit/pstoedit.exe" % (self.tmp_path)
        #out = exec_command([strPstoedit, '-help'],
